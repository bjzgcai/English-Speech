...
Configuration lives in `/srv/englisheval/shared/monitor.env`. Install from
...
The recipient is Wu Yanbiao (`<DINGTALK_USER_ID>`). The EnglishEval application and its
configured robot code are `<DINGTALK_ROBOT_CODE>`. Only in-app DING is used.
...
```sh
APP_ROOT=/srv/englisheval/current node scripts/monitor-control.js status
APP_ROOT=/srv/englisheval/current node scripts/monitor-control.js maintenance 30
APP_ROOT=/srv/englisheval/current node scripts/monitor-control.js resume
```
