...
| `DINGTALK_APP_KEY` / `DINGTALK_APP_SECRET` | Shared application credentials used to obtain the access token |
...
Each run logs one JSON line (`date`, `pv`, `uv`, `newUsers`, `dataIntegrity`,
`delivery`, `code`). It never logs the DING body or a credential. A delivery
